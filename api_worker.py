...
class CryptoLease:
    ...
    async def run_owned(self, function: Callable[..., Result], *args: Any, on_cancel: Callable[[], None]) -> Result:
        """Keep caller-owned files alive until actual work stops after cancellation."""
        future = asyncio.wrap_future(self._submit(function, *args))
        return await finish_owned(future, on_cancel)

    def _submit(self, function: Callable[..., Result], *args: Any) -> Future[Result]:
        worker = self._worker
        with worker._lock:
...
async def finish_owned(future: asyncio.Future[Result], on_cancel: Callable[[], None]) -> Result:
    """Delay cancellation cleanup until a shielded executor future really finishes."""
    interrupted = False
    while not future.done():
        try:
            ...
            break
    if interrupted:
        # Consume a worker failure too; the caller still observes its cancellation.
