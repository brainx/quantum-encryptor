...
- State-changing browser requests now require exact allowed Host/Origin equality, Origin-less cookie requests are rejected unless they use the explicit token header, and Vite development is an explicit `QUANTUM_ENCRYPTOR_ENABLE_VITE_DEV=1` exception for `http://127.0.0.1:4001` only.
